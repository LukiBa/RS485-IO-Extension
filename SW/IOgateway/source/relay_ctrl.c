...
eSetRelayStates_t setRelayState(eAvailableRelays_t id, eRelayStates_t newState)
{
	...
	{
		g_relayArray[id].state = newState;
		GPIO_PinWrite(g_relayArray[id].gpioType,g_relayArray[id].gpioPinNbr,newState);
	}
	if (BLINDS_UP == g_relayArray[id].type || BLINDS_DOWN == g_relayArray[id].type)
	{
		if(RELAY_ON == newState) // Turn off complementary RELAY of RELAY shall turn on
		{
			eAvailableRelays_t complementaryId = g_relayArray[id].complementaryRelayId;
			if (RELAY_ON == g_relayArray[complementaryId].state)
			{
				g_relayArray[complementaryId].state = RELAY_OFF;
				GPIO_PinWrite(g_relayArray[complementaryId].gpioType,
						g_relayArray[complementaryId].gpioPinNbr,RELAY_OFF);
